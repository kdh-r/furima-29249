class BuyAddress
  ...
  attr_accessor :user_id, :item_id, :token, :postal_code, :prefecture_id, :city, :address, :building, :phone_number
      ...
      validates :phone_number, format:{ with: /\A\d{10}\z|\A\d{11}\z/} #ハイフン不要の11桁以内の電話番号
      validates :prefecture_id, numericality: { other_than: 0, message: "can't be blank" } #都道府県が選択されていないといけない
